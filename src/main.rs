...
fn cmd_serve(config: skillguard::server::ServerConfig) -> Result<()> {
    ...
    if let Some(ref addr) = config.pay_to {
...
fn main() {
    ...
    let result = match cli.command {
        Commands::Serve {
            ...
            use skillguard::server::ServerConfig;
            ...
            match bind_addr {
                ...
                Ok(bind_addr) => {
                    ...
                    let config = ServerConfig {
                        ...
                        cache_dir,
                        api_key: std::env::var("SKILLGUARD_API_KEY").ok(),
                        pay_to: std::env::var("SKILLGUARD_PAY_TO").ok(),
                        facilitator_url: std::env::var("SKILLGUARD_FACILITATOR_URL")
                            .unwrap_or_else(|_| "https://pay.openfacilitator.io".to_string()),
                        external_url: std::env::var("SKILLGUARD_EXTERNAL_URL").ok(),
                        price_usdc_micro,
                    };
                    cmd_serve(config)
                }
