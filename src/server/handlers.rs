...
/// Build a JSON error response, recording elapsed time.
fn error_json(
    state: &ServerState,
    start: Instant,
    msg: String,
...
pub async fn evaluate_handler(
    ...
    let bytes = match axum::body::to_bytes(body, MAX_BODY_BYTES).await {
        Ok(b) => b,
        Err(e) => {
            return error_json(&state, start, format!("Failed to read request body: {}", e));
        }
    };
    ...
    let parsed: serde_json::Value = match serde_json::from_slice(&bytes) {
        Ok(v) => v,
        Err(e) => {
            return error_json(&state, start, format!("Invalid JSON: {}", e));
        }
    };
    ...
    if parsed.get("skill").map(|s| s.is_string()).unwrap_or(false) {
        ...
        let name_req: EvaluateByNameRequest = match serde_json::from_value(parsed) {
            Ok(r) => r,
            Err(e) => {
                return error_json(&state, start, format!("Invalid request: {}", e));
            }
        };
            ...
            .await
        {
            ...
            Err(e) => {
                ...
                return error_json(
                    &state,
                    start,
                    format!("Failed to fetch skill '{}': {}", name_req.skill, e),
                );
            }
        };

        if let Err(msg) = check_skill_content_size(&skill) {
            return error_json(&state, start, msg);
        }
