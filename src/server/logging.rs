...
pub struct UsageMetrics {
    ...
    redis_last_synced: Arc<std::sync::Mutex<HashMap<String, u64>>>,
}
...
impl UsageMetrics {
    ...
    pub fn new(access_log_path: &str, max_access_log_bytes: u64, cache_dir: &str) -> Self {
        ...
        Self {
            ...
            redis: None,
            rt_handle: None,
            redis_last_synced: Arc::new(std::sync::Mutex::new(HashMap::new())),
        }
    }
    ...
    pub async fn new_async(
        ...
        let metrics = Self {
            ...
            redis: redis_conn,
            rt_handle,
            redis_last_synced: Arc::new(std::sync::Mutex::new(last_synced)),
        };
    ...
    /// Return a reference to the AtomicU64 counter for a given field name.
    fn counter_ref(&self, field: &str) -> &AtomicU64 {
        match field {
            ...
            "deny" => &self.deny,
            "flag" => &self.flag,
            "ep_evaluate" => &self.ep_evaluate,
            "ep_evaluate_by_name" => &self.ep_evaluate_by_name,
            "ep_verify" => &self.ep_verify,
    ...
    /// `last_synced` is updated only after each successful HINCRBY so that
    /// failed deltas are retried on the next persist cycle.
    fn persist_to_redis(&self) {
        ...
        if let Ok(last_synced) = self.redis_last_synced.lock() {
        ...
        let last_synced = Arc::clone(&self.redis_last_synced);
        ...
        handle.spawn(async move {
            ...
            for (field, delta) in &deltas {
                ...
                match result {
                    Ok(_) => {
                        if let Ok(mut ls) = last_synced.lock() {
                            let prev = ls.get(field).copied().unwrap_or(0);
                            ls.insert(field.clone(), prev + delta);
                        }
                    }
                    Err(e) => {
