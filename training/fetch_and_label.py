...
def load_scan_report(
    path: str,
    prioritize_dangerous: bool = False,
    prioritize_boundary: bool = False,
) -> list[dict]:
    ...
    If prioritize_boundary is True, sorts skills by how confused the model
    is between SAFE and CAUTION (smallest abs(SAFE - CAUTION) first).
    This yields the most informative examples for fixing SAFE/CAUTION
    boundary errors.
    """
    ...
    for r in results:
        ...
        entry = {
            ...
            "dangerous_score": scores.get("DANGEROUS", 0.0),
            "safe_score": scores.get("SAFE", 0.0),
            "caution_score": scores.get("CAUTION", 0.0),
        }
    ...
    elif prioritize_boundary:
        # Sort by ascending |SAFE - CAUTION| score distance.
        # Skills where the model is most confused between SAFE and CAUTION
        # come first — these are the most valuable for boundary enrichment.
        entries.sort(key=lambda e: abs(e["safe_score"] - e["caution_score"]))
...
def main():
    ...
    parser.add_argument(
        "--prioritize-boundary", action="store_true",
        help="Sort skills by SAFE/CAUTION score proximity from scan-report before fetching. "
             "Fetches skills where the model is most confused between SAFE and CAUTION first, "
             "to improve the SAFE/CAUTION decision boundary."
    )
    ...
    if not args.existing_only:
        ...
        scan_skills = load_scan_report(
            args.scan_report,
            prioritize_dangerous=args.prioritize_dangerous,
            prioritize_boundary=args.prioritize_boundary,
        )
        ...
        if args.prioritize_boundary and verbose:
            # Show how many skills have close SAFE/CAUTION scores
            n_close = sum(1 for s in scan_skills if abs(s["safe_score"] - s["caution_score"]) < 0.2)
            print(f"  Prioritizing boundary: {n_close} skills with |SAFE-CAUTION| < 0.2 sorted to front")
        if verbose:
